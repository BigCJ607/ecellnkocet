...
import { fetchAndExportEventTeamsCSV } from '../utils/csvExporter'
...
export default function AdminPage() {
  const { user } = useApp()
  const [activeTab, setActiveTab] = useState<'events' | 'enrollments' | 'users' | 'system' | 'chats' | 'analytics'>('events')
  ...
  const handleExportEnrollmentsCSV = () => {
    ...
    const headers = ['Pass ID', 'Event Title', 'Student Name', 'PNR', 'Email', 'Branch', 'Class Year', 'Division', 'Team Name', 'Status', 'Enrolled At']
    ...
    link.setAttribute('download', `event_enrollments_${Date.now()}.csv`)
  ...
  const filteredUsers = users.filter(u => {
    ...
    return matchesSearch && matchesDiv && matchesBranch && matchesYear
  })
  ...
  const filteredEnrollments = enrollments.filter(enr => {
    ...
    const matchesSearch = !q || 
      ...
      enr.pnr.toLowerCase().includes(q) || 
      enr.branch.toLowerCase().includes(q) || 
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen flex items-center justify-center p-6" style={{ background: 'var(--color-bg)', paddingTop: 'calc(var(--nav-h) + 2rem)' }}>
        <div
          className="card-glass max-w-lg w-full p-10 text-center relative overflow-hidden"
          style={{
            borderColor: 'rgba(239, 68, 68, 0.4)',
            boxShadow: '0 20px 80px rgba(239, 68, 68, 0.15)',
          }}
        >
          <div
            className="w-16 h-16 mx-auto mb-6 flex items-center justify-center font-display text-2xl text-red-400"
            style={{
          ...
          <span className="font-ui font-semibold text-xs tracking-widest text-red-400 block mb-2">
            403 · RESTRICTED ACCESS
          </span>
          <h1 className="font-display text-4xl text-white mb-4">ADMIN CLEARANCE REQUIRED</h1>
          <p className="text-gray-400 text-sm mb-8 font-ui tracking-wide leading-relaxed">
            {user
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {!user ? (
              <Link to="/auth" className="btn-primary px-8 py-3 text-xs font-bold tracking-widest no-underline inline-block">
                SIGN IN AS ADMIN →
              </Link>
            ) : (
              <Link to="/" className="btn-primary px-8 py-3 text-xs font-bold tracking-widest no-underline inline-block">
                RETURN TO PLATFORM →
              </Link>
            )}
          </div>
  ...
  return (
    <div className="min-h-screen relative" style={{ background: 'var(--color-bg)', paddingTop: 'calc(var(--nav-h) + 2rem)', paddingBottom: '6rem' }}>
      {/* Toast Notification */}
      ...
      <div className="page-container">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <Link
            to="/"
            className="font-ui font-semibold text-xs tracking-widest text-gray-400 hover:text-white inline-flex items-center gap-2 no-underline"
          >
            ← BACK TO PLATFORM
          </Link>
          ...
          <div className="flex items-center gap-3">
            <span
              className="px-3 py-1 font-ui font-semibold text-xs tracking-widest inline-flex items-center gap-2"
              style={{
        ...
        <div className="flex flex-wrap items-end justify-between gap-6 mb-10">
          <div>
            <p className="font-ui font-semibold text-xs tracking-widest mb-2" style={{ color: 'var(--color-primary)', letterSpacing: '0.25em' }}>
            ...
            <button
              ...
              style={{
                color: activeTab === tab.id
                  ? (tab.id === 'chats' ? '#f59e0b' : 'var(--color-accent)')
                  : 'var(--color-text-muted)',
                background: activeTab === tab.id
                  ? (tab.id === 'chats' ? 'rgba(245,158,11,0.06)' : 'rgba(34,211,238,0.06)')
                  : 'transparent',
                border: 'none',
              }}
            >
              {tab.label}
              {activeTab === tab.id && (
                <span className="absolute bottom-0 left-0 right-0 h-0.5" style={{ background: tab.id === 'chats' ? '#f59e0b' : 'var(--color-accent)' }} />
              )}
            </button>
          ))}
        </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
              <div className="flex flex-wrap items-center gap-3 flex-1 max-w-xl">
                <input
                  type="text"
                  placeholder="Search enrolled students by name, PNR, email, branch, or team..."
                            ...
                            <table className="w-full text-left border-collapse">
                              <thead>
                                <tr className="border-b bg-white/[0.015]" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
                                  ...
                                  <th className="p-4 font-ui font-semibold text-xs tracking-widest text-gray-400">PNR / STUDENT ID</th>
                              ...
                              <tbody className="divide-y divide-white/[0.04]">
                                {group.enrollments.map((enr) => (
                                  <tr key={enr.id} className="hover:bg-white/[0.015] transition-colors">
                                    ...
                                    <td className="p-4">
                                      <p className="font-ui text-xs text-white font-semibold">{enr.branch || 'Branch Unassigned'}</p>
            ...
            <div className="mb-5">
              <div className="relative">
                ...
                <input
                  ...
                  type="text"
                  autoFocus
                  placeholder="Search students by name, email, PNR, branch, year, division..."
                  value={userSearch}
              ...
              <div className="flex flex-wrap items-center gap-2 p-2.5 bg-white/[0.02] border border-white/[0.06] rounded-xl">
                ...
                {branches.slice(0, 6).map((b) => (
                  <button
                    ...
                    style={{
                      background: userBranchFilter === b ? 'rgba(99,102,241,0.3)' : 'rgba(255,255,255,0.03)',
                      border: `1px solid ${userBranchFilter === b ? 'var(--color-primary)' : 'rgba(255,255,255,0.08)'}`,
                      color: userBranchFilter === b ? '#c7d2fe' : '#6b7280',
                    }}
                  >
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b bg-white/[0.02]" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
                        ...
                        <th className="p-4 font-ui font-semibold text-xs tracking-widest text-gray-400">PNR</th>
                        ...
                        <tr key={u.id} className="hover:bg-white/[0.015] transition-colors">
                          ...
                          <td className="p-4 text-right">
                            <button
                              ...
                              className="px-3 py-1 font-ui text-xs tracking-wider text-gray-400 hover:text-white border border-white/10 hover:border-white/30 cursor-pointer"
                            >
                    ...
                    {filtered.map(({ team, members }) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="w-full py-2 font-ui font-semibold text-xs tracking-widest cursor-pointer transition-all flex items-center justify-center gap-2"
                          style={{
                            background: 'rgba(245,158,11,0.1)',
                            border: '1px solid rgba(245,158,11,0.3)',
                            color: '#f59e0b',
          ...
          <div className="card-glass w-full max-w-2xl p-8 max-h-[85vh] flex flex-col overflow-hidden border border-cyan-500/30">
            ...
            <div className="flex-1 overflow-y-auto space-y-6">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="p-4 bg-black/40 border border-white/10">
                  <span className="font-ui text-xs text-gray-500 block mb-1">STUDENT PNR / PRN</span>
